...
class FMExperiment(object):
    def __init__(self, wideresnet, params, cta=None):
        ...
        ### Chengming added
        if self.params.decay_type == 'cosine':
            self.scheduler = get_cosine_schedule_with_warmup(self.optimizer, warmup_steps, total_training_steps)
            logger.info("I'm using cosine decay!")
        else:
            self.scheduler = scheduler = ABEL(self.optimizer, self.params.abel_decay)
            logger.info("I'm using ABEL!")
        #save log
    ...
    def save_checkpoint(self,state, epoch_idx):
        ...
        # Chengming edited to only save 1 file (the latest file)
        #filename = os.path.join(saving_checkpoint_file_folder,'{}_epoch_{}.pth.tar'.format(self.params.name, epoch_idx))
        filename = os.path.join(saving_checkpoint_file_folder,'{}.pth.tar'.format(self.params.name))
